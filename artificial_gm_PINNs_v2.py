...
def plot_result(Tn,x,x_data,y_data,yh,xp=None):
    ...
    plt.loglog(Tn[samp],sa_pred,color='red',linewidth=0.5, label="PINNs matching")
    plt.scatter(Tn[samp], sa_pred, s=60, color="tab:orange", alpha=0.4, label='Random points')
...
for i in range(max_epoch):
    ...
    #     sa_pred=rs_generate(Tn[samp],t,dt,gm_pred)
    #     plot_result(Tn,x,x_data,y_data,yh)
        
    #     file = "plots/pinns_%.8i.png"%(i+1)
...
# save_gif_PIL("pinns.gif", files, fps=20, loop=0)
